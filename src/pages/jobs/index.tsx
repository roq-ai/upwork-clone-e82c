...
import { useRoqClient, useJobFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<JobInterface, unknown>;
...
export function JobListPage(props: JobListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'application.count'],
  ...
  const { data, error, isLoading, mutate } = useJobFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.job.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: JobInterface) => {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/jobs/view/${record.id}`} passHref legacyBehavior>
                <Button
