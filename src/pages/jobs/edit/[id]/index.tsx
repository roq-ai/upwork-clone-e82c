...
import { useRoqClient, useJobFindFirst } from 'lib/roq';
...
function JobEditPage() {
  ...
  const { data, error, isLoading, mutate } = useJobFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: JobInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.job.update({
        data: values as RoqTypes.job,
        where: {
          id,
        },
      });
