...
import { useRoqClient, useApplicationFindManyWithCount } from 'lib/roq';
...
export function ApplicationListPage(props: ApplicationListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useApplicationFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.application.delete({ where: { id } });
      await mutate();
    } catch (error) {
    ...
    hasAccess('job', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/jobs/view/${record.job?.id}`}>
              {record.job?.title}
            </Link>
          ),
        }
